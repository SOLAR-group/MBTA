...
if __name__ == "__main__":
                    ...
                    with codecs.open(java_output, 'r', encoding="unicode_escape") as output_file:
                        ...
                        if 'TIMEOUT' in output_lines[0]:
                            ...
                            if any("EXCEPTION" in element for element in output_lines):
                                ...
                                split_line = line.split('/')
                                mutant_dir = directory + '/mutants/' + line
                                output_dir = directory + '/output/' + str(
                                    Path(line).parent.with_suffix(".csv")).replace("\\", "-")
                                python_mutant = Path(mutant_dir).with_suffix('.py')
                                if os.path.isfile(python_mutant) and os.path.getsize(python_mutant) > 0:
                                    num_python_mutants += 1
                                    python_compiled = Path(mutant_dir).with_suffix('.pyc')
                                    if os.path.isfile(python_compiled) and os.path.getsize(python_compiled) > 0:
                                        num_python_compiled += 1
                                        python_output = Path(output_dir).with_name(Path(output_dir).stem + "_python.csv")
                                        if os.path.isfile(python_output) and os.path.getsize(python_output) > 0:
                                            num_python_runnable += 1
                                            with codecs.open(python_output, 'r') as output_file:
                                                output_lines = output_file.readlines()
    ...
    print(
        f'Number of Java Compiled Mutants: {num_java_compiled} ({round(num_java_compiled / num_java_mutants * 100, 2)}%)')
    ...
    print(
        f'Number of Java 10 outputs: {num_java_10_outputs} ({round(num_java_10_outputs / num_java_mutants * 100, 2)}%)')
    ...
    print(
        f'Number of Python Compiled Mutants: {num_python_compiled} ({round(num_python_compiled / num_java_success * 100, 2)}%)')
    print(
        f'Number of Python Runnable: {num_python_runnable} ({round(num_python_runnable / num_java_success * 100, 2)}%)')
    print(
        f'Number of Python Timeout: {num_python_time_out} ({round(num_python_time_out / num_java_success * 100, 2)}%)')
    print(
        f'Number of Python 10 outputs: {num_python_10_outputs} ({round(num_python_10_outputs / num_java_success * 100, 2)}%)')
    print(
        f'Number of Python Exceptions: {num_python_exception} ({round(num_python_exception / num_java_success * 100, 2)}%)')
