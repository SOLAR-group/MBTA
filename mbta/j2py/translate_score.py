...
if __name__ == "__main__":
    ...
    percentage_of_passing_test_cases_per_class = merged.loc[merged['equal_results'] == True].groupby(
        'class').size() / merged.groupby('class').size()
    percentage_of_passing_test_cases_per_class = percentage_of_passing_test_cases_per_class.to_frame(name='percentage')

    percentage_of_passing_test_cases_per_mutant = merged.loc[merged['equal_results'] == True].groupby(
        ['mutant']).size() / merged.groupby(['mutant']).size()
    percentage_of_passing_test_cases_per_mutant = percentage_of_passing_test_cases_per_mutant.to_frame(
        name='percentage')
    ...
    print(
        f'Unique classes with no failures: {len(classes_with_no_failure)}/{number_of_classes} ({len(classes_with_no_failure) / float(number_of_classes) * 100:#.2f}%)')
    print(
        f'Unique operators with no failures: {len(operators_with_no_failure)}/{number_of_operators} ({len(operators_with_no_failure) / float(number_of_operators) * 100:#.2f}%)')

    percentage_of_passing_test_cases_per_class.rename(columns={'percentage': 'Percentage'}).boxplot(
        column=['Percentage'], grid=False)

    sb.violinplot(data=percentage_of_passing_test_cases_per_class,
                  y=percentage_of_passing_test_cases_per_class['percentage'])

    percentage_of_passing_test_cases_per_mutant = merged.loc[merged['equal_results'] == True].groupby(
        ['mutant', 'class']).size() / merged.groupby(['mutant', 'class']).size()
    percentage_of_passing_test_cases_per_mutant = percentage_of_passing_test_cases_per_mutant.to_frame(
        name='percentage')
    box_plot = percentage_of_passing_test_cases_per_mutant.rename_axis(index={'mutant': "Mutation Operator"}).boxplot(
        column=['percentage'], grid=False, by="Mutation Operator", rot=45)
