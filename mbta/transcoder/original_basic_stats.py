...
if __name__ == "__main__":
    directory = '../..'
    with open(f'{directory}/intersecting_classes.txt', 'r') as mutants_file:
    ...
    for line in progress:
        line = line.replace('\n', '')
        original_dir = directory + '/data/java/' + line
        ...
        if os.path.isfile(java_mutant) and os.path.getsize(java_mutant) > 0:
            ...
            if os.path.isfile(java_compiled) and os.path.getsize(java_compiled) > 0:
                num_java_compiled += 1
                java_output = directory + '/data/java/' + line + '.csv'
        ...
        python_mutant = original_dir + '_TRANSLATED.py'
        if os.path.isfile(python_mutant) and os.path.getsize(python_mutant) > 0:
            num_python_programs += 1
            python_output = directory + '/data/java/' + line + '_TRANSLATED.csv'
    ...
    print(f'Number of Python Compiled Programs: {num_python_compiled}')
